...
function ExplorePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const list = await fetchBuilds();  // array from server
        setBuilds(list);
      } catch {
  ...
  return (
    <main className="page-shell explore-shell">
      <section className="section-header">
        <h1 className="section-title"><span className="section-bar" /> Explore Builds</h1>
        <p className="section-desc">Browse community builds. Open any card to view specs, parts, dyno numbers, and runs.</p>

        <div className="explore-filters-row">
          <div className="chips-row">
            {chipOptions.map((c) => (
              <button key={c} type="button" className={`chip ${tag === c ? "active" : ""}`} onClick={() => setTag(c)}>
                {c}
              </button>
            ))}
          </div>
          ...
          <div className="sort-wrap">
            ...
            <select id="sort" className="sort-select" value={sortKey} onChange={(e) => setSortKey(e.target.value)}>
        ...
        {visible.map((b) => (
          <article className="build-card" key={b.id}>
            <div
              ...
              style={{ backgroundImage: `url(${b.bg})` }}   // server provides string URLs
              role="button"
            ...
            <div className="build-card-body">
              ...
              <div className="chip-row-tight">
                {b.chips?.map((c) => <span key={c} className="chip dark">{c}</span>)}
              </div>
              <button className="btn-view" onClick={() => { setSelected(b); setOpen(true); }}>View Build</button>
            </div>
          </article>
        ))}
      </section>
